...
export class CacheProviderRegistry extends null {
    static #providers: Map<string, CacheProvider> = new Map([ ["memory", new InMemoryCacheProvider()] ]);
    ...
    static resolveCacheProvider(name: string): CacheProvider {
        if (!this.#providers.has(name)) {
            throw createQuartzError(QuartzErrorCode.CACHE_PROVIDER_NOT_FOUND, { providerName: name });
        }
